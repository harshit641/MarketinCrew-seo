...
import { importBacklinksCsvAction, type ImportSummary } from "@/app/(app)/backlinks/actions";

export function ImportBacklinksButton({ clientId }: { clientId: string }) {
  ...
  async function handleFile(file: File) {
    ...
    start(async () => {
      const res = await importBacklinksCsvAction(clientId, text);
      if (res.ok) {
        setSummary(res.data);
        router.refresh();
      } else {
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-[var(--radius)] border border-border bg-card p-6 shadow-lg">
            ...
            <h2 className="text-lg font-semibold">Import backlinks</h2>
            <p className="mt-1 text-sm text-muted-foreground">Existing backlinks (same source + target URL) are updated.</p>
            <div className="mt-4 rounded-md border border-dashed border-border p-6 text-center">
              <FileSpreadsheet className="mx-auto h-8 w-8 text-muted-foreground" />
              <p className="mt-2 text-xs text-muted-foreground">
                Columns: source_url, target_url, anchor_text, link_type, status, domain_rating, first_seen, acquired, cost, vendor, campaign, method
              </p>
